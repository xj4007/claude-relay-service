...
class ResponseCacheService {
  ...
  /**
   * 生成缓存键（基于请求内容的唯一哈希）
   * ⚠️ 重要：必须包含 sessionHash 以确保会话隔离，防止不同会话共享缓存
   * sessionHash 已经包含了 apiKeyId，因此也确保了用户隔离
   *
   * @param {Object} requestBody - 请求体
   * @param {string} model - 模型名称
   * @param {string} sessionHash - 会话哈希 (必须传入以确保缓存隔离)
   * @returns {string} - 缓存键
   */
  generateCacheKey(requestBody, model, sessionHash) {
    if (!sessionHash) {
      logger.warn(
        `⚠️ Cache key generation without sessionHash - this may cause cache sharing between sessions!`
      )
      return null
    }
    ...
    try {
      ...
      logger.debug(`📋 Cache key generated: ${hash} (session: ${sessionHash.substring(0, 8)}...)`)
      return hash
    } catch (error) {
