...
基于请求内容和会话信息生成唯一的缓存键（SHA256 哈希）：

```javascript
缓存键 = hash(
  sessionHash + // 会话哈希（包含 apiKeyId + sessionId）
...
- **sessionHash**: 会话哈希，由 `sessionHelper.generateSessionHash()` 生成
  - 格式：`SHA256(apiKeyId + sessionId + cacheableContent)`
  - 确保：同一会话的请求可以共享缓存
  - 隔离：不同会话即使用同一 API Key 也不会共享缓存
  - 用户隔离：sessionHash 中包含 apiKeyId，确保不同用户的缓存完全隔离
...
```javascript
// 旧逻辑 (已废弃)：基于 apiKeyId
cacheKey = SHA256(apiKeyId + model + messages + ...)

// 新逻辑 (当前)：基于 sessionHash
cacheKey = SHA256(sessionHash + model + messages + ...)
```

#### 隔离保证

1. **用户隔离**：sessionHash 包含 apiKeyId，不同用户的缓存完全隔离
2. **会话隔离**：同一用户的不同会话有不同的 sessionHash
3. **会话共享**：同一会话的重复请求可以命中缓存

#### sessionHash 生成规则

优先级从高到低：

1. **metadata.user_id** 中的 session UUID（如果存在）
2. **cache_control** 标记的可缓存内容
3. **system** prompt 内容
4. **第一条消息**内容

```javascript
// 示例：从 Claude Code 请求中提取
user_id = 'user_abc123_account__session_def456-789a-bcde-f012-3456789abcde'
sessionId = 'def456-789a-bcde-f012-3456789abcde'
sessionHash = SHA256(apiKeyId + '_' + sessionId)
```
...
```javascript
// claudeConsoleRelayService.js:265-288
const isStreamRequest = requestBody.stream === true
// 🔒 生成会话哈希（包含 apiKeyId 确保用户隔离）
...
# 检查日志中的会话信息
tail -f logs/claude-relay-*.log | grep "Cache key generated"
# 应该看到：📋 Cache key generated: abc123... (session: def456...)
```
...
```bash
# 查看日志，确认不同会话生成不同的缓存键
# 相同内容，不同会话 → 不同 sessionHash → 不同 cacheKey
# 相同内容，相同会话 → 相同 sessionHash → 相同 cacheKey → 缓存命中！
```
...
- ✅ **重要改进**：缓存键从 `apiKeyId` 改为 `sessionHash`
  - 更精确的会话级缓存（同一会话可共享，不同会话隔离）
  - 保持用户隔离（sessionHash 包含 apiKeyId）
  - 提高缓存命中率（相同会话的重复请求）
- ✅ 添加 `sessionHelper` 集成
- ✅ 更新日志输出（显示 session 信息）
- ✅ 完善文档说明（用户和会话隔离机制）
